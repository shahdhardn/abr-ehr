...
def input_fn_builder(input_files,
  ...
  def input_fn(params):
    ...
    #         tf.io.FixedLenFeature([max_seq_length], tf.int64, default_value= [0] * max_seq_length),
    #     "masked_lm_positions":
    #         tf.io.FixedLenFeature([max_predictions_per_seq], tf.int64, default_value= [0] * max_predictions_per_seq),
    #     "masked_lm_ids":
    #         tf.io.FixedLenFeature([max_predictions_per_seq], tf.int64, default_value= [0] * max_predictions_per_seq),
    #     "masked_lm_weights":
    #         tf.io.FixedLenFeature([max_predictions_per_seq], tf.float32, default_value= [0.0] * max_predictions_per_seq),
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    train_spec = tf.compat.v1.estimator.TrainSpec(input_fn=train_input_fn, max_steps=FLAGS.num_train_steps)
    eval_spec = tf.compat.v1.estimator.EvalSpec(input_fn=eval_input_fn, throttle_secs=0)
    # added by YX, evaluate and get the result
    result = tf.compat.v1.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
    print(result)
    result = result[0]
    #### end of added part
