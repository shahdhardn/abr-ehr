...
class MimicDataModule(pl.LightningDataModule):
    ...
    def note_feats(self):
        ...
        notes = pd.read_csv(self.mimic_dir / 'NOTEEVENTS.csv.gz', parse_dates=['CHARTDATE', 'CHARTTIME', 'STORETIME'], compression = "gzip")

        icus = pd.read_csv(self.mimic_dir / 'ICUSTAYS.csv.gz', parse_dates=['INTIME', 'OUTTIME'], compression = "gzip").sort_values(
    ...
    def split(self, file, k):
        df_label = pd.read_csv(self.label_dir). \
            rename(columns={'ID': 'ICUSTAY_ID', f'{self.task}_LABEL': 'LABEL'})
        if self.notes:
