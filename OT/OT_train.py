...
parser.add_argument(
    "--print-freq", "-p", default=100, type=int, help="print frequency (default: 10)"
)
...
weightsbuffer = torch.tensor(
    [per_cls_weights[cls_i] for cls_i in imbalanced_train_dataset.targets]
).to("cuda")
        ...
        if is_best:
            ...
            for i_cls in range(args.num_classes):
                weightsbuffer_bycls.extend(
                    weightsbuffer[imbalanced_train_dataset.targets == i_cls]
                    .data.cpu()
                    .numpy()
                )
...
def train_OT(
    ...
    if args.meta_set == "whole":
        ...
        val_data_bycls = torch.zeros(
            [args.num_classes, args.num_meta, 3, 32, 32]
        ).cuda()
        for i_cls in range(args.num_classes):
